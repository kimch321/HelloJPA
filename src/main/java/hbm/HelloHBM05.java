...
public class HelloHBM05 {
    public static void main(String[] args) {
        ...
        try {
            ...
            // query.add(Expression.like("fname","eve")); // 안나옴
            // query.add(Expression.like("fname","eve", MatchMode.START)); // eve%
            // query.add(Expression.like("fname","eve", MatchMode.END)); // %eve
            query.add(Expression.like("fname","eve", MatchMode.ANYWHERE)); // %eve%

            List<Employee> emps = query.list();
            System.out.println(emps);*/

            // 서브쿼리: 부서명이 IT인 사원들의 이름, 직책, 입사일 조회
            // DetachedCritetia로 서브쿼리 정의 후
            // Critetia로 정의된 주 쿼리에 부착해서 실행
